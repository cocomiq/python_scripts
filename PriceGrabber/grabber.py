    ...
    for lnk in item_list:
        ...
        if WebDriverWait(browser, 10, 0.25).until(
            ...
            items.append(lnk[0], item_details.find("a", {"class":"manufacturer"}).text, "https://www.gratis.com" + item_details.find("a", {"class":"manufacturer"})["href"],
                ...
                promos, campaigns, item_stickers.find("img", {"class":"product-sticker product-sticker_top-right"})["src"],
                sw_items, ri_items)
...
m = get_gratis_main_links(browser, url)

sc, i = get_gratis_sub_links(browser, m)


links_all = []

itemDetails = pd.DataFrame(items, columns = [
